...
const Gameboard = () => {
  ...
  const isReserved = (y, x, length, direction) => {
    for (let i = 0; i < length; i++) {
      if (
        (direction === 'v' && board[y + i][x].occupant) ||
  ...
  const isValidPlace = (y, x, len, dir) => {
    if (dir === 'v') {
      if (x < 0 || x >= boardSize || y < 0 || y + len > boardSize) return false;

      for (let i = -1; i < len + 1; i++) {
        if (y + i >= 0 && y + i < boardSize) {
          if (isReserved(y + i, x, 1, 'v')) return false;
          if (x + 1 < boardSize && isReserved(y + i, x + 1, 1, 'v'))
            return false;
          if (x - 1 >= 0 && isReserved(y + i, x - 1, 1, 'v')) return false;
        }
      }
    } else if (dir === 'h') {
      if (x < 0 || x + len > boardSize || y < 0 || y >= boardSize) return false;

      for (let i = -1; i < len + 1; i++) {
        if (x + i >= 0 && x + i < boardSize) {
          if (isReserved(y, x + i, 1, 'h')) return false;
          if (y + 1 < boardSize && isReserved(y + 1, x + i, 1, 'h'))
            return false;
  ...
  const shipsCount = () => {
    ...
    for (let i = 0; i < board.length; i++) {
      for (let j = 0; j < board[0].length; j++) {
        if (board[i][j].occupant && board[i][j].shipIndex === 0) count++;
      }
    }
    return count;
  };
